...
used by all annotation source providers (PDF, Calibre Viewer, Kindle,
Zotero, Kobo, etc.).
"""
...
class Annotation:
    ...
    source: str  # "kindle", "kobo", "apple_books", "pdf", "calibre_viewer", "zotero"
    ...
    source_doc_id: Optional[str] = None  # Adapter doc_id (set after matching)
    calibre_id: Optional[int] = None  # Kept for backwards compatibility
...
class AnnotationProvider(ABC):
    ...
    @abstractmethod
    def name(self) -> str:
        """Unique provider name (e.g. 'kindle', 'pdf', 'zotero')."""
        ...
