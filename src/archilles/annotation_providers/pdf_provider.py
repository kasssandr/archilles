...
class PdfAnnotationProvider(AnnotationProvider):
    ...
    def extract(self, path: str, **kwargs) -> list[Annotation]:
        ...
        # Suppress MuPDF's C-level stderr for corrupt PDFs; restore afterwards.
        try:
        ...
        try:
            ...
            for page_num in range(total_pages):
                try:
                    ...
                    logger.debug("Skipping page %d in %s: %s", page_num + 1, path, page_exc)
                    continue
            ...
            logger.warning("Could not extract PDF annotations from %s: %s", path, e)
        finally:
            try:
